...
function Test({snakeArr,setSnakeArr,highScore,setHighScore,difficulty,volume}) {
...
if(snakeArr[0].x>18 || snakeArr[0].y>18 || snakeArr[0].x<=0 || snakeArr[0].y<=0){
  ...
  function generateNewFoodPosition(){
    ...
    const newFood = {x:Math.floor((Math.random()*17)+1),y:Math.floor((Math.random()*18)+1)}
...
function trackSnake(){
  ...
  if(snakeArr[0].x===food.x && snakeArr[0].y===food.y){
    console.log(snakeArr[0].x===food.x && snakeArr[0].y===food.y)
  console.log("Swallow",snakeArr[0].x);
    foodSound.play();
  // console.log("ponit",food.y+1,snakeArr[0].y)
  //I get true how
